...
RegisterNetEvent('jim-recycle:server:toggleItem', function(give, item, amount, newsrc)
	...
	if give == 0 or give == false then
		local hasItemWithAmounts = exports['qb-inventory']:HasItem(src, item, remamount)
		if hasItemWithAmounts then
			local ItemRemoved = exports['qb-inventory']:RemoveItem(src, item, remamount, false, "Jim-Recycling")
		else
			--Has No Items
		end
		...
		if exports['qb-inventory']:AddItem(src, item, amount, false, false, "Jim Recycle") then
